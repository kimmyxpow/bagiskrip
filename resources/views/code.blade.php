...
<section class="grid grid-cols-12 sm:gap-6 gap-1 py-8">
   <div class="col-span-10 col-start-2 space-y-5">
      <div class="space-y-2">
         ...
         <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center sm:gap-0 gap-4">
            <div class="flex items-center gap-4">
               <p class="font-poppins text-mycolor-light-text flex items-center gap-1"><img class="rounded-full object-cover object-center" src="{{ $code->user->avatar }}" style="width: 30px; height: 30px; min-height: 30px; min-width: 30px; max-width: 30px; max-height: 30px;" alt="{{ $code->user->username }}">{{ $code->user->username }}</p>
            ...
            <div class="flex gap-1 copy-clipboard">
               <input class="border border-gray-200 rounded-xl py-2 px-6 font-poppins text-mycolor-dark-text focus:border-mycolor focus-within:border-mycolor focus-within:outline-none w-full" type="text" readonly value="http://127.0.0.1:8000/codes/{{ $code->slug }}">
