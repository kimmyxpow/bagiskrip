...
<section class="grid sm:grid-cols-12 grid-cols-1 sm:gap-6 gap-1 py-8 sm:px-0 px-4">
   <form method="POST" action="/codes/{{ $code->slug }}" class="sm:col-span-10 col-span-1 sm:col-start-2 col-start-1 space-y-5">
      @method('put')
      @csrf
         ...
         <div class="flex flex-col gap-1">
            ...
            <select class="border border-gray-200 rounded-xl py-2 px-6 font-poppins cursor-pointer text-mycolor-dark-text focus:border-mycolor focus-within:border-mycolor focus-within:outline-none @error('lang') border-red-500 @enderror" name="lang" id="selectBahasa">
               ...
               @foreach ($langs as $lang)
                   <option value="{{ $lang['id'] }}" {{ ($lang['id'] == old('lang', $code->lang->id)) ? 'selected' : '' }}>{{ $lang['name'] }}</option>
               @endforeach
            </select>
            @error('lang')
      ...
      <div class="flex flex-col gap-1">
         ...
         <textarea class="border border-gray-200 rounded-xl focus:outline-none focus:border-mycolor resize-none font-mono text-mycolor-dark-text py-3 px-4 h-96 @error('code') border-red-500 @enderror" name="code" id="textareaCode" placeholder="Ketikkan kode disini">{!! old('code', $code->code) !!}</textarea>
         @error('code')
             <small class="italic font-poppins font-medium text-red-500">{{ $message }}</small>
         @enderror
      </div>
      <div class="flex flex-col gap-4">
         <div class="flex flex-col gap-1 sm:max-w-max">
            <label class="font-bold font-montserrat text-mycolor-dark-text" for="neededPassword">Tambah kata sandi baru:</label>
         ...
         <div class="hidden flex-col gap-1 sm:max-w-max">
            <label class="font-bold font-montserrat text-mycolor-dark-text" for="password">Kata sandi baru:</label>
      ...
      <div class="flex flex-col gap-4">
         <div class="flex flex-col gap-1 sm:max-w-max">
            ...
            <input value="{{ old('name', $code->name) }}" autocomplete="off" class="border border-gray-200 rounded-xl py-2 px-6 font-poppins text-mycolor-dark-text focus:border-mycolor focus-within:border-mycolor focus-within:outline-none @error('name') border-red-500 @enderror" type="text" name="name" id="name">
            @error('name')
                <small class="italic font-poppins font-medium text-red-500">{{ $message }}</small>
            @enderror
         </div>
