<nav class="sticky top-0 grid grid-cols-12 space-x-6 gap-6 py-3 bg-white" style="z-index: 999;">
               ...
               @auth
                  <div class="relative">
                     ...
                     <div class="absolute w-40 right-0 bg-mycolor rounded-xl flex flex-col overflow-hidden opacity-0 pointer-events-none invisible transition-all duration-300" id="menu-dropdown">
                        <a class="text-white flex items-center space-x-1 gap-1 font-poppins hover:bg-blue-500 py-2.5 px-5 transition-all duration-300" href="/codes">
                           <i class="bx bx-code text-xl"></i>
                           Kode Saya
                        </a>
                        <form method="POST" action="{{ route('logout') }}">
                           @csrf
                           <a class="text-white flex items-center space-x-1 gap-1 font-poppins hover:bg-blue-500 py-2.5 px-5 transition-all duration-300" href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();">
